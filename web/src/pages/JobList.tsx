...
import { cn, fmtDur, jobStateLabel, modeLabel } from "../lib/utils";
  ...
  return (
    <div className="grid h-full min-h-0 grid-cols-1 overflow-y-auto lg:grid-cols-[minmax(19rem,25rem)_minmax(0,1fr)_minmax(17rem,22rem)] lg:overflow-hidden">
        ...
        {active ? (
          <div className="mx-auto max-w-3xl">
            ...
            <div className="mt-8 rounded-xl border border-line bg-surface p-5">
              ...
              <div className="mt-5 flex flex-wrap gap-2 text-xs text-muted"><span className="rounded border border-line px-2 py-1">{active.source?.kind === "url" ? "URL 输入" : "本地输入"}</span><span className="rounded border border-line px-2 py-1">{String(active.options?.generate_path || "i2va_turbo")}</span><span className="rounded border border-tungsten/40 bg-tungsten/10 px-2 py-1 text-tungsten">{modeLabel(active.mode)}</span></div>
            </div>
      ...
      <aside className="min-h-0 overflow-y-auto border-l border-line bg-panel/40 p-4">
        ...
        <div className="mt-5 space-y-3 text-sm"><div className="flex items-center justify-between border-b border-line pb-3"><span className="text-muted">执行模式</span><span className="text-tungsten">{modeLabel("mock")}</span></div><div className="flex items-center justify-between border-b border-line pb-3"><span className="text-muted">素材</span><span>内置 Fixture</span></div><div className="flex items-center justify-between border-b border-line pb-3"><span className="text-muted">模型调用</span><span className="text-ok">已模拟</span></div><div className="flex items-center justify-between"><span className="text-muted">GPU</span><span className="text-muted">无需</span></div></div>
